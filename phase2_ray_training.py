...
Run: python phase2_ray_training.py

What this adds:
  - Ray Train distributed training (multi-GPU / multi-node)
  - Ray Tune hyperparameter optimisation (replaces basic Optuna HPO)
  - Automatic device detection and data parallel training
  - Fault-tolerant checkpointing
  - Writes src/training/distributed.py
"""
...
DISTRIBUTED_TRAINING = '''"""src/training/distributed.py — Distributed training with Ray Train + Ray Tune."""
from __future__ import annotations
...
def main() -> None:
    console.print("[bold cyan]Phase 2 — Distributed Training with Ray[/]\n")

    out = Path("src/training/distributed.py")
    out.write_text(DISTRIBUTED_TRAINING, encoding="utf-8")
    console.print(f"  [green]Written[/] → {out}")

    # Write launch script
    launch = Path("scripts/train_distributed.py")
    launch.write_text('''"""Launch distributed training with Ray."""
from __future__ import annotations
import sys, typer
from pathlib import Path
...
if __name__ == "__main__":
    typer.run(main)
''', encoding="utf-8")
    console.print(f"  [green]Written[/] → {launch}")

    console.print("\n[bold green]Phase 2 complete![/]")
    console.print("  src/training/distributed.py      — Ray Train + Ray Tune")
    console.print("  scripts/train_distributed.py     — launch script")
    console.print("\nUsage:")
    console.print("  [cyan]pip install ray[train][/]")
    console.print("  [cyan]python scripts/train_distributed.py --workers 4 --gpu[/]")
    console.print("  [cyan]python scripts/train_distributed.py --hpo --samples 20[/]")
    console.print("\n[dim]Next: python phase3_feature_store.py[/]")
