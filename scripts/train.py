...
def main(
    ...
    splits_needed = ["train.parquet", "val.parquet", "test.parquet"]
    if not all((feat_path / s).exists() for s in splits_needed):
    ...
    if val_df.empty:
        raise RuntimeError(
            f"Val split is empty ({feat_path / 'val.parquet'}). "
            "Add more raw data or re-run the feature pipeline after fixing the split ratios."
        )
    common_cols = [c for c in train_df.columns if c in val_df.columns]
    if not common_cols:
        raise RuntimeError(
            f"Train and val parquet files share no columns. "
            f"Train cols: {list(train_df.columns)}. Val cols: {list(val_df.columns)}."
        )
    text_col = "text" if "text" in common_cols else common_cols[0]
    label_col = "label" if "label" in common_cols else common_cols[-1]
    ...
    # 5. Register & promote (best-effort — skipped if MLflow tracking server is unavailable)
    try:
        import mlflow
        ...
        console.print(f"[yellow]Model registry skipped:[/] {exc}")
