...
class TextDataset(Dataset):
    def __init__(self, texts: list[str], labels: list[Any], tokenizer: Any, max_length: int = 128) -> None:
        if not texts:
            raise ValueError("TextDataset received an empty texts list — check that the feature split is non-empty.")
        # Coerce to str; replace NaN/None with empty string
...
class Trainer:
    ...
    def train(
        ...
        # Best-effort MLflow setup — training continues even if tracking is unavailable.
        _mlflow_ok = False
        _active_run = None
        try:
            mlflow.set_tracking_uri(tracking_uri)
            mlflow.set_experiment(self.cfg.experiment_name)
            _active_run = mlflow.start_run(run_name=run_name or self.cfg.experiment_name)
            _active_run.__enter__()
            mlflow.log_params(self.cfg.model_dump())
            _mlflow_ok = True
        except Exception as exc:
            logger.warning(f"MLflow tracking unavailable ({exc}). Continuing without experiment tracking.")
        ...
        try:
            ...
            if _mlflow_ok and _active_run is not None:
                try:
                    _active_run.__exit__(None, None, None)
                except Exception:
                    pass
