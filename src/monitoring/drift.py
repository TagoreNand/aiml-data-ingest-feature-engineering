...
class DriftDetector:
    ...
    def _psi(expected: pd.Series, actual: pd.Series, buckets: int = 10) -> float:
        ...
        if min_val == max_val:
            return 0.0
        buckets = min(buckets, max(2, len(expected) - 1))
        breakpoints = list(np.linspace(min_val - 1e-9, max_val + 1e-9, buckets + 1))
        ...
        import math
        return float(((expected_pct - actual_pct) * (expected_pct / actual_pct).apply(math.log)).sum())
