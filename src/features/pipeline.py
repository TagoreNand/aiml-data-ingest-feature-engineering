...
class FeaturePipeline:
    ...
    def validate(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        n_before = len(df)
        # Exclude unhashable columns (dicts/lists) from duplicate check
        hashable_cols = [
            col for col in df.columns
            if not df[col].apply(lambda x: isinstance(x, (dict, list))).any()
        ]
        df = df.drop_duplicates(subset=hashable_cols if hashable_cols else None)
    ...
    def split(self, df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        ...
        n = len(df)
        # Guarantee at least 1 row in val and test even for tiny datasets
        val_size = max(1, int(n * self.cfg["data"]["val_split"]))
        test_size = max(1, int(n * self.cfg["data"]["test_split"]))
    ...
    def materialise(self, df: pd.DataFrame, split: str = "train") -> Path:
        ...
        if "payload" in df.columns:
            ...
            base_df = df.drop(columns=["payload"]).reset_index(drop=True)
            # Drop payload columns that already exist in base to avoid duplicates
            new_cols = [c for c in payload_df.columns if c not in base_df.columns]
            payload_df = payload_df[new_cols]
            df = pd.concat([base_df, payload_df], axis=1)
